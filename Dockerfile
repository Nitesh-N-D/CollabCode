...
COPY packages/student-portal/package.json packages/student-portal/package.json
...
FROM base AS student-portal
ARG VITE_SERVER_URL=http://localhost:4000
ENV VITE_SERVER_URL=$VITE_SERVER_URL
RUN pnpm --filter @collabcode/shared build && pnpm --filter @collabcode/student-portal build
EXPOSE 5174
CMD ["pnpm", "--filter", "@collabcode/student-portal", "preview", "--", "--port", "5174"]
